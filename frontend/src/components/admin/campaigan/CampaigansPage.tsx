...
const CampaigansPage = () => {
  ...
  const {discounts , cancelCampaigan , fetchDiscounts , isLoading} = useDiscountsStore()
  const discount = discounts.find(discount=>discount.code === 'campaigan')
   const handleCancelCampaigan = async ()=>{
      const response = await cancelCampaigan()
      if (response.success){
      ...
      {discount ? (
        <div className="bg-base-100 p-6 rounded-xl shadow-lg mb-6">
          <div className="space-y-4">
            ...
            >
             {isLoading ? <FaSpinner className="animate-spin transition-all duration-200" /> :  " لغو کمپین"}
            </motion.button>
          </div>
        </div>
      ) : (
